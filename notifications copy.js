const authToken = localStorage.getItem('token') || "eyJhbGciOiJodHRwOi8vd3d3LnczLm9yZy8yMDAxLzA0L3htbGRzaWctbW9yZSNobWFjLXNoYTI1NiIsInR5cCI6IkpXVCJ9..CZoFccrC_eBIvc60zLk9cwokS29MI8KPYdfGrX7KKiw";
...
let flexWrapper = document.getElementById('flex_wrapper');
...
if (!topbarContainer) {
    ...
    flexWrapper.appendChild(topbarContainer);
}
...
notificationIcon.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" color="var(--mantine-color-primary-light-color)" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-bell">
        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
...
function fetchNotifications(limit = 10) {
    const requestBody = {
        ...
        iLimit: 0,
        iLimit2: limit,
        iCMID: loginUser?.customerId || "",
        cmName: loginUser?.name || "Vincent Fabris - Apollo.courses Admin",
    ...
    if (loginUser) {
        fetch('https://devapi.apollo.business/api/Common/GetAllNotifications', {
            ...
            headers: {
                ...
                'Authorization': `Bearer ${authToken}` // Add authToken to headers
            },
            body: JSON.stringify(requestBody)
        })
            .then(response => response.json())
            .then(data => {
                updateNotifications(data, limit);
    ...
    if (data.messages && data.messages.length > 0) {
        ...
        `;
            messageItem.addEventListener('click', function () {
                markAsRead(i, message);
            });
            dropdownBody.appendChild(messageItem);
        });

        // updateUnreadCount(data.nrUnreadMessages); // Update unread count

        if (data.messages.length >= limit) {
            readMoreButton.style.display = 'inline-flex'; // Show "Read more" button if more than or equal to limit messages
        } else {
            readMoreButton.style.display = 'none'; // Hide "Read more" button if fewer than limit messages
        }
    } else {
...
const connection = new signalR.HubConnectionBuilder()
    .withUrl('https://signalrdev.apollo.business/chatHub') // Use the URL from chat.js
...
connection.on('ReceiveConversationsMessage', function (user, message) {
    ...
    unreadMessagesCount += 1; // Increment unread count
    updateUnreadCount(unreadMessagesCount); // Update unread count
});
                ...
                fetch('https://devapi.apollo.business/api/Common/MarkNotificationAsRead', {
                    ...
                    .then(data => {
                        if (!data.success) {
                            ...
                            const configData = JSON.parse(message.configData);
                            if (configData && configData.redirectURL) {
...
window.addEventListener('click', function (event) {
    ...
    if ((dropdown && !dropdown.contains(event.target)) && (notificationIcon && !notificationIcon.contains(event.target))) {
        if (dropdown.classList.contains('show')) {
            dropdown.classList.remove('show');
            setTimeout(() => {
                dropdown.style.display = 'none';
