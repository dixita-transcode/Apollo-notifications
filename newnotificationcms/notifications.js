document.addEventListener('DOMContentLoaded', function () {
    function loadScript(url) {
        const showNotificationsOnlySites = ['51', '52']
        const siteId = window?.SiteID?.toString() || window?.siteID?.toString()
        if (!showNotificationsOnlySites.includes(siteId)) return;
    ...
    `;
        ...
        function fetchNotifications(limit = 10) {
            ...
            const requestBody = {
                ...
                iLimit2: limit,
                iCMID: window.CMID,
                cmName: window.CMName,
                iSiteID: window.SiteID,
                outputCasing: "pascal"
            };
        ...
        // (function () {
        //     if (!window._webSocketOverridden) { // Check if the override has already been applied
        ...
        //             if (idParam) {
        //                 console.log("Extracted WebSocket ID:", idParam);
        //                 window.webSocketId = idParam; // Save the WebSocket ID globally or pass it to a function later
        //             } else {
        //                 console.log("No ID parameter found in WebSocket URL.");
        //             }
        ...
        //         window._webSocketOverridden = true; // Mark the override as applied
        //     }
        // })();
        ...
        async function saveNotificationConnectionId(connectionId) {
            const payload = {
            ...
            try {
                const response = await fetch('https://devapi.apollo.business/api/Common/SaveNotificationConnectionId', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (!response.ok) {
                    throw new Error('Failed to save connection ID');
                }

                console.log("Connection ID saved successfully.");
                return response;
            } catch (error) {
        ...
        async function start() {
            if (connection.state === signalR.HubConnectionState.Disconnected) {
                try {
                    ...
                    setTimeout(async () => {
                        console.log('///', connection.connection.transport.constructor.name === "WebSocketTransport");

                        if (connection.connection.transport.constructor.name === "WebSocketTransport") {
                            const transportUrl = connection.connection.transport.webSocket.url;
                            const id = new URL(transportUrl).searchParams.get('id');
                            const result = await saveNotificationConnectionId(id);
                            if (result.status === 200) {
                                setupMessageListener();
                            } else {
                                console.error("Failed to save notification connection ID");
                            }
                        } else {
                            console.error("WebSocket transport not available or different transport type in use.");
                        }
                ...
                console.log("SignalR connection is in", connection.state, "state. Waiting...");
        ...
        function setupMessageListener() {
            ...
            connection.on("ReceiveConversationsMessage", function (user, message) {
                ...
                if (window.CMID === user.data.audienceCustomerId[0].toString()) {
                    notificationsData.unshift(newMessage);
                    updateUnreadCount(unreadMessagesCount + 1);
        ...
        function markAsRead(messageId, message) {
            ...
            if (messageItem) {
                ...
                const configData = message.configData && JSON.parse(message.configData);
